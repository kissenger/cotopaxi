...
app.post('/save-created-route/', auth.verifyToken, (req, res) => {
  ...
  path.init().then( () => {
    mongoModel('route').create( path.asMongoObject(req.userId, true) ).then( (doc) => {
...
app.get('/get-path-by-id/:type/:id', auth.verifyToken, (req, res) => {
  ...
  getPathDocFromId(req.params.id, req.params.type, req.userId).then( doc => {
...
app.get('/write-path-to-gpx/:type/:id', auth.verifyToken, (req, res) => {
  ...
  mongoModel(req.params.type).find({_id: req.params.id, userId: req.userId}).then( document => {
