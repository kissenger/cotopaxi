...
export class UnitPipe implements PipeTransform {
  transform(value: number, type: string, unitA: string, unitB?: string): string {
    ...
    // if value is a lumpiness (elevation/height), baseline is m/km
      ...
      if (unitA === 'm') {
        if (unitB === 'km' ) { return value.toFixed(2) + 'm/km'}
        else if (unitB === 'miles') { return (value / KM_2_MI).toFixed(2) + 'm/mi'}
      } else if (unitA === 'ft') {
        if (unitB === 'km' ) { return (value * M_2_FT).toFixed(2) + 'ft/km'}
        else if (unitB === 'miles') { return (value * M_2_FT / KM_2_MI).toFixed(2) + 'ft/mi'}        
      }
    }
  }
}
