...
import { TsCoordinate, TsElevationQuery, TsElevationResults } from 'src/app/shared/interfaces';
...
export class GeoService {
  ...
  */
  ...
  getElevationsFromAPI(coordsArray: Array<TsCoordinate>, interpolate: boolean) {
    ...
    return new Promise<Array<number>>( (resolve, reject) => {
      ...
      sliceArray.reduce( (promise, coords) => {
        return promise.then( (allResults: TsElevationResults) =>
          this.elevationQuery({options: {interpolate}, coords}).then( (thisResult: TsElevationResults) =>
            [...allResults, thisResult]
          ));
      }, Promise.resolve([]) ).then( (result: Array<TsElevationResults>) => {
        resolve(result[0].map( (e: TsCoordinate) => e.elev) );
      });
  ...
  elevationQuery(query: TsElevationQuery) {
    return new Promise<TsElevationResults>( (res, rej) => {
      this.httpService.myElevationsQuery(query).subscribe( (elevs: {result: TsElevationResults}) => {
        res(elevs.result);
      });
    });
  }
